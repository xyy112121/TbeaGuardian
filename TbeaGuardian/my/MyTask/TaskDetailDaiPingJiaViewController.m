//
//  TaskDetailDaiPingJiaViewController.m
//  TbeaGuardian
//
...
#import "TaskDetailDaiPingJiaViewController.h"

@interface TaskDetailDaiPingJiaViewController ()

@end

@implementation TaskDetailDaiPingJiaViewController
...
-(UIView *)viewcell:(CGRect)frame Dicbid:(NSDictionary *)dic IndexPath:(NSIndexPath *)indexpath
{
    ...
    lablemoney.text = [dic objectForKey:@"taskstatus"];
    ...
    UIButton *button = [AddCustomView CusViewButtonForRadius:CGRectMake(SCREEN_WIDTH-nowwidth*2, 10, nowwidth*2-10, 40) BGColor:[UIColor clearColor] Title:@"去评价" TitleColor:COLORNOW(255,127,0) FRadius:0.0f];
    button.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
    [button addTarget:self action:@selector(clickgotopingjia:) forControlEvents:UIControlEventTouchUpInside];
...
{
    NSDictionary *dictemp = [FCbidlist objectAtIndex:0];
    MyTaskDetailPingJiaViewController *mytask = [[MyTaskDetailPingJiaViewController alloc] init];
    mytask.FCtaskperson = dictemp;
    mytask.FCapplyuserid = [dictemp objectForKey:@"userid"];
    mytask.FCtaskId =_FCtaskid;
    mytask.FCtaskcode = FCtaskcode;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    if(section == 3)
    {
        ...
        [buttonmiddle updatebuttonitem:EnButtonTextCenter TextStr:@"任务状态" Font:FONTN(14.0f) Color:[UIColor blackColor] Image:nil];
...
#pragma mark 接口
//获取已接单任务详情
-(void)gettaskdaipingjiadetail
{
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"taskid"] = _FCtaskid;
    
    [RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:RQMyTaskDetailDaiPingJiaCode ReqUrl:InterfaceRequestUrl ShowView:app.window alwaysdo:^{
        ...
        if([[dic objectForKey:@"success"] isEqualToString:@"true"])
        {
            ...
            FCtaskcode = [[[dic objectForKey:@"data"] objectForKey:@"taskinfo"] objectForKey:@"taskcode"];
