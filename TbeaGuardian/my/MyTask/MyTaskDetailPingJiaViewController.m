//
//  MyTaskDetailPingJiaViewController.m
//  TbeaGuardian
//
...
#import "MyTaskDetailPingJiaViewController.h"

@interface MyTaskDetailPingJiaViewController ()

@end

@implementation MyTaskDetailPingJiaViewController
...
-(void)Clicksave:(id)sender
{
    ...
    {
        [MBProgressHUD showError:@"请填写完全评价数据" toView:app.window];
    }
    else
    {
        [self uploadtaskpingjiainfo];
    }
}
    ...
    switch (indexPath.section) {
            ...
            switch (indexPath.row)
        {
            ...
            case 1:
                ...
                [imageview setImageWithURL:[NSURL URLWithString:[_FCtaskperson objectForKey:@"thumbpicture"]] placeholderImage:LOADIMAGE(@"testpic", @"png")];
                
                [cell.contentView addSubview:imageview];
                
                textfield.frame = CGRectMake(XYViewR(imageview)+5, 10, 170, 30);
                textfield.text = [_FCtaskperson objectForKey:@"personname"];
                [cell.contentView addSubview:textfield];
                break;
        }
            break;
...
-(void)uploadtaskpingjiainfo
{
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"taskid"] = _FCtaskId;
    params[@"applyuserid"] = _FCapplyuserid;
    params[@"starlevel"] = FCStarLevel;
    params[@"appraise"] = FCAppraise;
    params[@"picturelist"] = FCPictureList;
    DLog(@"params===%@",params);
    
    [RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:RQMyTaskSendPingJiaCode ReqUrl:InterfaceRequestUrl ShowView:app.window alwaysdo:^{
