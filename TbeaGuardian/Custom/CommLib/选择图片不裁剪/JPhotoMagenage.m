...
+ (void)getTakePhotoImageInController:(UIViewController *)viewController
                         finish:(void(^)( UIImage *images))finish
                         cancel:(void(^)())cancel
{
    JPhotoMagenage *manage = [self shareInstance];
    manage.single = YES;
    [manage showActionTakePhoto:viewController];
    manage.completeOneBlock = finish;
    manage.cancelBlock = cancel;
}


+ (void)getTakeLibryImageInController:(UIViewController *)viewController
                               finish:(void(^)( UIImage *images))finish
                               cancel:(void(^)())cancel
{
    JPhotoMagenage *manage = [self shareInstance];
    manage.single = YES;
    [manage showActionTakeLibray:viewController];
    manage.completeOneBlock = finish;
    manage.cancelBlock = cancel;
}
...
- (void)showActionTakePhoto:(UIViewController *)viewController{
...
- (void)showActionTakeLibray:(UIViewController *)viewController{
