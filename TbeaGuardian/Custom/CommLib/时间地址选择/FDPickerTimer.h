...
typedef NS_ENUM (NSInteger, PickerButtonLocation) {
    PickerButtonLocationBottom, // 1.选择器在视图的下方
    PickerButtonLocationCenter  // 2.选择器在视图的中间
};
@interface FDPickerButton : UIButton
/** 1.内部视图 */
...
/** 6.视图的显示模式 */
@property (nonatomic, assign) PickerButtonLocation FDPickerButtonLocation;
...
@protocol PickerTimerDelegate <NSObject>

- (void)pickerTimer:(FDPickerTimer *)pickerTimer year:(NSInteger)year month:(NSInteger)month day:(NSInteger)day;

@end
@interface FDPickerTimer : FDPickerButton

@property (nonatomic,weak) id<PickerTimerDelegate> FDDelegate;
...
@interface NSCalendar (FDPickerTimerCalendar)
/**
 *  1.当前的日期数据元件模型
 *
 */
...
@interface UIView (FDPickerTimerView)
/**
 *  1.间隔X值
 */
