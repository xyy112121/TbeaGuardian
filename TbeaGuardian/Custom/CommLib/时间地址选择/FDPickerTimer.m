...
//  timePicker
//
//  Created by xuansa on 2017/1/6.
//  Copyright © 2017年 xuansa. All rights reserved.
//

#import "FDPickerTimer.h"
#import "STUI.h"
///** 1.统一的较小间距 5*/
//CGFloat  STMarginSmall = 5;
//
///** 2.统一的间距 10*/
//CGFloat  STMargin = 10;
//
///** 3.统一的较大间距 16*/
//CGFloat  STMarginBig = 16;
...
@interface FDPickerTimer () <UIPickerViewDataSource, UIPickerViewDelegate>
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    NSString *text;
    if (component == 0) {
        text = [NSString stringWithFormat:@"%ld年",(long)(row + 1900)];
    }else if (component == 1){
        text = [NSString stringWithFormat:@"%ld月",(long)(row + 1)];
    }else{
        text = [NSString stringWithFormat:@"%ld日",(long)(row + 1)];
    }
...
- (void)selectedOk{
    if ([self.FDDelegate respondsToSelector:@selector(pickerTimer:year:month:day:)]) {
        [self.FDDelegate pickerTimer:self year:self.year month:self.month day:self.day];
    }
    [super selectedOk];
}
...
- (void)setupDefault{
    ...
    _heightPicker = 240;
    _FDPickerButtonLocation = PickerButtonLocationBottom;
...
- (void)layoutSubviews{
    ...
    if (self.FDPickerButtonLocation == PickerButtonLocationBottom) {
...
- (void)setFDPickerButtonLocation:(PickerButtonLocation)FDPickerButtonLocation{
    _FDPickerButtonLocation = FDPickerButtonLocation;
    if (FDPickerButtonLocation == PickerButtonLocationCenter) {
...
- (UIButton *)buttonLeft {
    if (!_buttonLeft) {
		...
		_buttonLeft.layer.borderColor = [UIColor clearColor].CGColor;
      //  _buttonLeft.backgroundColor = COLORNOW(0, 170, 238);
...
- (UIButton *)buttonRight {
    if (!_buttonRight) {
		...
		_buttonRight.layer.borderColor = [UIColor clearColor].CGColor;
      //  _buttonRight.backgroundColor = COLORNOW(0, 170, 238);
...
@implementation NSCalendar (FDPickerTimerCalendar)
...
@implementation UIView (FDPickerTimerView)
